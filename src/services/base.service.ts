...
export abstract class BaseService<T> {
  ...
  protected async create(data: Omit<T, 'id'>): Promise<T> {
    const { isAuthenticated } = useAuth();
    if (!isAuthenticated) {
      throw new Error(`User must be authenticated to create a ${this.resource}`);
    }
  ...
  protected async update(data: T & { id: number }): Promise<T> {
    const { isAuthenticated } = useAuth();
    if (!isAuthenticated) {
      throw new Error(`User must be authenticated to update a ${this.resource}`);
    }

    const response = await this.axiosInstance.put<T>(`/${this.resource}/${data.id}`, data);
    return response.data;
  }
  ...
  protected async delete(id: number): Promise<void> {
    const { isAuthenticated } = useAuth();
    if (!isAuthenticated) {
      throw new Error(`User must be authenticated to delete a ${this.resource}`);
    }
