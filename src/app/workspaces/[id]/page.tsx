...
export default function WorkspaceDetail({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="container mx-auto p-4">
      <Link href="/workspaces" className="inline-flex items-center text-blue-600 mb-6">
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex space-x-2">
          <Link href={`/workspaces/${workspace.id}/edit`} className="p-2 bg-gray-200 rounded-md">
            <IconEdit size={18} />
          </Link>
          <button onClick={handleDelete} className="p-2 bg-red-100 text-red-600 rounded-md">
            <IconTrash size={18} />
          </button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          <span className={`px-3 py-1 rounded-full text-sm ${getPriorityColor(workspace.priority)}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-lg shadow-sm p-6">
          <div className="flex justify-between items-center mb-4">
            ...
            <Link href={`/workspaces/${workspace.id}/lists/new`} className="inline-flex items-center px-3 py-1 bg-blue-600 text-white rounded-md">
          ...
          {lists.length === 0 ? (
            <div className="bg-gray-100 p-4 rounded-lg text-center">
              <p className="text-gray-600">No lists found. Create your first list!</p>
            </div>
          ) : (
              ...
              {lists.map((list) => (
                <div key={list.id} className="border rounded-lg p-4">
                  ...
                  <div className="flex justify-end mt-2 space-x-2">
                    <Link href={`/workspaces/${workspace.id}/lists/${list.id}/edit`} className="text-gray-600 hover:text-gray-900">
                      <IconEdit size={16} />
                    </Link>
                    <button 
                      onClick={() => {
        ...
        <div className="bg-white rounded-lg shadow-sm p-6">
          <div className="flex justify-between items-center mb-4">
            ...
            <Link href={`/workspaces/${workspace.id}/tasks/new`} className="inline-flex items-center px-3 py-1 bg-blue-600 text-white rounded-md">
              <IconPlus size={16} className="mr-1" />
              Add Task
            </Link>
          </div>
            ...
            <div className="space-y-4">
              {tasks.map((task) => (
                <div key={task.id} className="border rounded-lg p-4">
                  ...
                  <div className="flex justify-end mt-2 space-x-2">
                    <Link href={`/workspaces/${workspace.id}/tasks/${task.id}/edit`} className="text-gray-600 hover:text-gray-900">
                      <IconEdit size={16} />
                    </Link>
                    <button 
                      onClick={() => {
