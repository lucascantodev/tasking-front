...
{
    return (
        <div className={cn("dark w-full bg-background text-foreground py-[3.4375rem] px-[2.5rem]", className)} 
            {...props}>
            {children}
        </div>
    );
...
{
    return (
        <span className={cn("text-[1.875rem]", className)} {...props}>{children}</span>
    );
}

export function Button({ href, target="_self", className, children, ...props }: RouteProps) 
{
    return (
        <Route 
            href={href}
...
export default function (
    { href, labelText, buttonText }: 
    Readonly<{ href: string, labelText: ReactNode | ReactNode[], buttonText: ReactNode | ReactNode[] }>
) 
{
    return (
        <Root>
